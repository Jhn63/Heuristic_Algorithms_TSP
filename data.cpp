...
void readData( int argc, char** argv, int* Dimension, double ***Mdist )
{
    ...
    //4- array com as coordenadas de cada cidade
    //ex: cidade 1 -> x[1] e y[1], cidade 2 -> x[2] e y[2] 
    ...
    if ( ewt == "EXPLICIT" ) {
        ...
        //6- ler coordenadas das cidades 
        int tempCity;
        for ( int i = 1; i < N+1; i++ ) {
            in >> tempCity >> x[i] >> y[i];
        }

        //7- Calcular Matriz Distancia (Euclidiana)
        for ( int i = 1; i < N+1; i++ ) {
            for ( int j = 1; j < N+1; j++ ) {
...
//3- metodo para preencher os atributos da classe data
